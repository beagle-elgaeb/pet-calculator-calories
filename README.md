...
- при необъодимости можно удалить запись.
...
- в карточке дня отображать данные о соотношении рассчитаного расхода и потреблении калорий;
- задать минимальное количество ккал, ниже которого нельзя опускаться.

## Директории

- `/src` — рабочая папка проекта;
- `/src/app` — непереиспользуемые компоненты приложения;
- `/src/components` — переиспользуемые компоненты приложения;
- `/src/redux` — менеджер состояний;
- `/src/styles` — переиспользуемые стили;
- `/src/utils` — типизация, схемы валидации, математические рассчёты, вспомогательные функции;
